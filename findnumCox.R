findnumCox <- function(target,event,time,confound,numcross,totalcut=3,initial_rr=NULL
  ...
  aictest.apply<-foreach(i = 1:1,.combine = cbind) %do% {
    ...
    if(is.null(confound)){
      counfound=NULL
    }else{confound=userdata$confound}
  ...
  lik_cut1=NULL;hr1=NULL
  lik_cut2=NULL;hr2=NULL
  lik_cut3=NULL;hr3=NULL
  pvalue1=NULL;pvalue2=NULL;pvalue3=NULL;
  cutpvalue1=NULL;cutpvalue2=NULL;cutpvalue3=NULL;
  #N <- numcross*simulation
  ...
  ##overall p-value
  overall_pvalue <- list(overallpvalue1 = sort(1-pchisq(2*pvalue1,1)),overallpvalue2 =sort(1-pchisq(2*pvalue2,2))
                         ,overallpvalue2 =sort(1-pchisq(2*pvalue3,3)))
  ##HR
  hr <- list(hr1=exp(median(hr1[,1])),hr2=c(exp(median(hr2[,1])),exp(median(hr2[,2])))
